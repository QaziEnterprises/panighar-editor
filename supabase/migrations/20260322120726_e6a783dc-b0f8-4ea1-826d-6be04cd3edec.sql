...
CREATE POLICY "Admins can insert backup history" ON public.backup_history FOR INSERT TO authenticated WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admins can insert receivable payments" ON public.receivable_payments FOR INSERT TO authenticated WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can delete receivable payments" ON public.receivable_payments FOR DELETE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
