    ...
    <div>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" variant="outline" className="gap-2" disabled={filtered.length === 0} onClick={() => {
            printAsPDF("Purchases", ["Date", "Supplier", "Ref No", "Status", "Total"],
              filtered.map(p => [p.date, getSupplierName(p.supplier_id), p.reference_no || "—", p.payment_status, `Rs ${Number(p.total).toLocaleString()}`])
            );
